...
export async function createEvent(eventData) {
  ...
  const canCreate = await canPerformAction(user.uid, 'events', eventData.date)

  if (!canCreate) {
    throw new Error(
      'You have reached your daily event limit. Please upgrade to Pro for unlimited events.'
    )
  }
  ...
  // Note: We don't increment monthly usage for events since they have daily limits
  // The daily limit is checked by counting actual events for the specific date
...
export async function deleteEvent(eventId) {
  ...
  // Note: We don't decrement monthly usage for events since they have daily limits
  // The daily limit is checked by counting actual events for the specific date
}
