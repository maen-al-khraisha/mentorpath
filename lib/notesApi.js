...
export async function createNote({ title, description, labels = [], userId }) {
  ...
  if (!canCreate) {
    throw new Error('You have reached your note limit. Please upgrade to Pro for unlimited notes.')
  }
  ...
  const note = {
    ...
    isActive: true, // Mark as active (similar to habits and sheets)
  }

  await db.add(NOTES_STORE, note)

  // Don't increment usage for notes (they are total active, not monthly)
  // Similar to habits and sheets - we count total active notes, not monthly usage
...
export async function deleteNote(noteId, userId = 'current-user') {
  ...
  // Don't decrement usage for notes (they are total active, not monthly)
  // Similar to habits and sheets - we count total active notes, not monthly usage
}
