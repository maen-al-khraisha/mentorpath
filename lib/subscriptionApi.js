...
export async function updateUserPlan(userId, plan, additionalData = {}) {
  ...
  if (plan === PLAN_TYPES.PRO) {
    ...
    // Use canonical package id: pro-plan (compat: basic_plan)
    updateData.current_package = 'pro-plan'
    updateData.package_name = await getPackageName('pro-plan')
  ...
  if (plan === PLAN_TYPES.FREE) {
    ...
    // Use canonical package id: free-plan (compat: basic-plan)
    updateData.current_package = 'free-plan'
    updateData.package_name = await getPackageName('free-plan')
    updateData.subscription_status = 'active'
  }
...
export async function canPerformAction(userId, action, date = null) {
  ...
  let userPackage = subscription.current_package || 'free-plan'
  if (userPackage === 'free' || userPackage === 'basic-plan') userPackage = 'free-plan'
  if (userPackage === 'pro' || userPackage === 'basic_plan' || userPackage === 'premium')
    userPackage = 'pro-plan'
