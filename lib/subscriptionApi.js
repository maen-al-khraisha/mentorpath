...
export async function createUserSubscription(userId, userData = {}) {
  ...
  const subscriptionData = {
    ...
    current_package: 'basic-plan', // Assign to free plan package
    package_name: await getPackageName('basic-plan'), // Set package name for display
...
export async function updateUserPlan(userId, plan, additionalData = {}) {
  ...
  // If upgrading to pro, set subscription dates and package info
  if (plan === PLAN_TYPES.PRO) {
    updateData.subscriptionStartDate = new Date()
    updateData.subscriptionEndDate = null // Pro plan doesn't expire
    updateData.current_package = 'basic_plan' // Set Pro package ID
    updateData.package_name = await getPackageName('basic_plan') // Set Pro package name dynamically
    updateData.subscription_status = 'active'
  }

  // If downgrading to free, clear subscription dates and set free package info
  if (plan === PLAN_TYPES.FREE) {
    updateData.subscriptionStartDate = null
    updateData.subscriptionEndDate = null
    updateData.downgradeDate = new Date()
    updateData.current_package = 'basic-plan' // Set Free package ID
    updateData.package_name = await getPackageName('basic-plan') // Set Free package name dynamically
    updateData.subscription_status = 'active'
  }
...
export async function canPerformAction(userId, action, date = null) {
  ...
  let userPackage = subscription.current_package || 'basic-plan'

  // Fix package ID mapping
  if (userPackage === 'free') {
    userPackage = 'basic-plan' // Use existing basic-plan package
  } else if (userPackage === 'pro') {
    userPackage = 'basic_plan' // Use existing basic_plan package
  ...
  try {
    ...
    if (!packageData) {
      console.warn(`Package ${userPackage} not found, denying action`)
      // Deny action if package not found
      return false
    }
...
export async function getPlanLimits(userId) {
  try {
    ...
    // Fix package ID mapping
    if (userPackage === 'free') {
      userPackage = 'basic-plan'
    } else if (userPackage === 'pro') {
      userPackage = 'basic_plan'
    } else if (userPackage === 'premium') {
      userPackage = 'basic_plan'
    }
    ...
    // Throw error if package not found
    throw new Error(`Package ${userPackage} not found`)
  } catch (error) {
