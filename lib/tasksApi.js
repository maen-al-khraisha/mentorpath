...
export async function createTask(input) {
  ...
  if (!canCreate) {
    throw new Error('You have reached your task limit. Please upgrade to Pro for unlimited tasks.')
  }
  ...
  const data = {
    ...
    attachments: [],
    isActive: true, // Mark as active (similar to habits, sheets, and notes)
  }

  const refDoc = await addDoc(tasksCol(), data)

  // Increment usage for tasks (monthly limit)
...
export async function deleteTask(id, userId = 'current-user') {
  ...
  // Decrement usage count when deleting a task (monthly limit)
  try {
